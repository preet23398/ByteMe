...
public class App {
    ...
    private static final String PENDING_ORDERS_FILE = "src/pendingOrders.txt";
    private static final String CUSTOMER_ORDERS_FILE = "src/customerOrders.txt";

    public static void appendOrderToCustomerOrdersFile(Order order) {
        try (FileWriter writer = new FileWriter(CUSTOMER_ORDERS_FILE, true)) {
            StringBuilder orderLine = new StringBuilder();
            orderLine.append(order.getCustomerName()).append(",");
            orderLine.append(formatItems(order.getItems()));

            writer.write(orderLine.toString() + "\n");
        } catch (IOException e) {
            System.out.println("Error writing to the customer orders file: " + e.getMessage());
        }
    }


    private static Map<String, Integer> parseItems(String itemsStr) {
    ...
    private static Map<String, String> parseSpecialRequests(String requestsStr) {
        Map<String, String> specialRequests = new HashMap<>();
        String[] requestsArray = requestsStr.split(",");
        for (String request : requestsArray) {
            String[] requestData = request.split(":");
            specialRequests.put(requestData[0], requestData[1]);
        }
    ...
    public static void writeAllOrdersToFile() {
        try (FileWriter writer = new FileWriter(PENDING_ORDERS_FILE)) {
            List<Order> allOrders = new ArrayList<>();
            allOrders.addAll(vipOrderQueue);
            allOrders.addAll(orderQueue);
            allOrders.addAll(vipCancelled);
            allOrders.addAll(regularCancelled);
            ...
            for (Order order : allOrders) {
                writer.write(order.getOrderId() + "," +
                        ...
                        formatSpecialRequests(order.getSpecialRequests()) + " \n");
            }
        } catch (IOException e) {
    ...
    private static String formatItems(Map<String, Integer> items) {
        ...
        for (Map.Entry<String, Integer> entry : items.entrySet()) {
            formattedItems.append(entry.getKey()).append(":").append(entry.getValue()).append(",");
        }
        return formattedItems.toString();
    }

    private static String formatSpecialRequests(Map<String, String> specialRequests) {
        StringBuilder formattedRequests = new StringBuilder();
        for (Map.Entry<String, String> entry : specialRequests.entrySet()) {
            formattedRequests.append(entry.getKey()).append(":").append(entry.getValue()).append(",");
        }
        return formattedRequests.toString();
    }
    ...
    private static void login(Scanner scanner) {
        ...
        HashMap<String, User> users = loadUsersFromFile();
        ...
        while (true) {
            ...
            switch (choice) {
                case 1:
                    ...
                    customer.addItemToCart(selectedItem, quantity, specialRequest);
                    break;
                ...
                case 5:
                    if (customer.getCart().isEmpty()) {
                        ...
                        if (newOrder != null) {
                            ...
                            appendOrderToCustomerOrdersFile(newOrder);
                        }
                    }
    ...
    private static void updateOrderStatus(Scanner scanner) {
        ...
        System.out.println("\nPending Orders:");
        List<Order> allOrdersList = new ArrayList<>(App.vipOrderQueue);
        allOrdersList.addAll(App.orderQueue);
        ...
        Order selectedOrder = allOrdersList.get(orderNumber - 1);
    ...
    private static void saveMenuToFile() {
        try {
            File menuFile = new File("src/menu.txt");  // Adjust file path if necessary
            ...
            for (Item item : menuItems) {
                writer.println(item.getName() + "," + item.getPrice() + "," + item.isAvailable());
            }
            writer.close();
        } catch (IOException e) {
            System.out.println("Error: Unable to save the menu to the file.");
            e.printStackTrace();
        }
